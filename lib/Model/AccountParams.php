 ...
 */
class AccountParams implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'AccountParams';
    ...
    */
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['account_name']) && (mb_strlen($this->container['account_name']) > 512)) {
            $invalidProperties[] = "invalid value for 'account_name', the character length must be smaller than or equal to 512.";
        }
    ...
    public function setAccountName($account_name)
    {
        ...
        if ((mb_strlen($account_name) > 512)) {
            throw new \InvalidArgumentException('invalid length for $account_name when calling AccountParams., must be smaller than or equal to 512.');
        }
        if ((mb_strlen($account_name) < 1)) {
            throw new \InvalidArgumentException('invalid length for $account_name when calling AccountParams., must be bigger than or equal to 1.');
        }
        if ((!preg_match("/[A-Za-z0-9¡-ʯ &\\(\\)\\{\\}\\[\\]\\.:,;\\?!\\+\\-_\\$@#~`\\^€]*/", $account_name))) {
     ...
     * Sets is_new
     *
     * @param bool|null $is_new Whether this account should be flagged as 'new' or not. Any newly imported account will have this flag initially set to true, and remain so until you set it to false (see PATCH /accounts/<id>). How you use this field is up to your interpretation, however it is recommended to set the flag to false for all accounts right after the initial import of the bank connection. This way, you will be able to recognize accounts that get newly imported during a later update of the bank connection, by checking for any accounts with the flag set to 'true' after every update of the bank connection.
     *
     * @return self
     */
