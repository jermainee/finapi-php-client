...
class ImportBankConnectionParams implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    */
     ...
     * Sets name
     *
     * @param string|null $name Custom name for the bank connection. If you do not want to set a name, you can leave this field unset.
     *
     * @return self
     */
    public function setName($name)
    {
        ...
        if ((mb_strlen($name) > 64)) {
            throw new \InvalidArgumentException('invalid length for $name when calling ImportBankConnectionParams., must be smaller than or equal to 64.');
        }
        if ((mb_strlen($name) < 1)) {
            throw new \InvalidArgumentException('invalid length for $name when calling ImportBankConnectionParams., must be bigger than or equal to 1.');
        }
    ...
    public function setRedirectUrl($redirect_url)
    {
        ...
        if ((mb_strlen($redirect_url) > 2048)) {
            throw new \InvalidArgumentException('invalid length for $redirect_url when calling ImportBankConnectionParams., must be smaller than or equal to 2048.');
        }
        if ((mb_strlen($redirect_url) < 1)) {
            throw new \InvalidArgumentException('invalid length for $redirect_url when calling ImportBankConnectionParams., must be bigger than or equal to 1.');
        }
