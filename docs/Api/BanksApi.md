...
$supported_banking_interfaces = array('supported_banking_interfaces_example'); // string[] | Comma-separated list of banking interfaces. Possible values: FINTS_SERVER,WEB_SCRAPER,XS2A. If this parameter is specified, then all the banks that support at least one of the given interfaces will be returned. Note that this does NOT imply that those interfaces must be the only ones that are supported by a bank.
...
try {
    $result = $apiInstance->getAndSearchAllBanks($ids, $search, $supported_banking_interfaces, $location, $tpp_authentication_group_ids, $is_test_bank, $page, $per_page, $order, $x_request_id);
    print_r($result);
} catch (Exception $e) {
...
| **supported_banking_interfaces** | [**string[]**](../Model/string.md)| Comma-separated list of banking interfaces. Possible values: FINTS_SERVER,WEB_SCRAPER,XS2A. If this parameter is specified, then all the banks that support at least one of the given interfaces will be returned. Note that this does NOT imply that those interfaces must be the only ones that are supported by a bank. | [optional] |
